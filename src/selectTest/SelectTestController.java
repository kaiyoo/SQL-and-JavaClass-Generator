...
public class SelectTestController implements Initializable {
    ...
    @FXML private VBox vBoxCheckBox;
	 ...
	 * @param event
	 */
	@FXML void exitSelect(KeyEvent event){
	...
	public void base(String tableName) throws Exception {
		...
		for (int i = 0; i < columnCnt; i++) {
			...
			vBoxCheckBox.getChildren().add(checkBox[i] = new CheckBox());
		}
		...
		btnGeneration.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				// 이제 템플릿 파일을 읽어보자.
				Template t = ve.getTemplate("/template/selectTest.vm");
				
				List<String> selectTemplate = new ArrayList<String>();
				...
				try {
					for (int i = 0; i < columnCnt; i++) {
						if(checkBox[i].isSelected()) {
							selectTemplate.add(selectTemplate.size() > 0 ?
									"\t, " + rsmd.getColumnName(i + 1) : "\t" + rsmd.getColumnName(i + 1));
						}
					...
					if (selectTemplate.size() == 0) {
						selectTemplate.add("*");
					}
					
					// 템플릿에 던져줄 데이터 만들기
					Map<String, Object> map = new HashMap<String, Object>();
					map.put("selects", selectTemplate);
